...
55 | timeless-1.0.ckpt`
  ...
  onStart: async function ({ message, api, args, event }) {
    ...
    const puti = 'xyz';
    let baseURL = `https://smfahim.${puti}/prodia?prompt=${prompt}&model=${models}&count=4`;
    ...
    try {
      ...
      message.reply({
        body: "Here are 4 generated images. Reply with the number of the image you like (1-4).",
        attachment: attachments
      });

      const listener = async (replyMessage) => {
        if (replyMessage.senderID === event.senderID) {
          const selectedImageIndex = parseInt(replyMessage.body, 10) - 1;
          if (selectedImageIndex >= 0 && selectedImageIndex < 4) {
            api.sendMessage({
              body: `Here is the image you selected:`,
              attachment: attachments[selectedImageIndex]
            }, event.threadID);
      ...
      api.listenMqtt((replyMessage) => listener(replyMessage));
    } catch (error) {
